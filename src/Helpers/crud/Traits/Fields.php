...
trait Fields
{
    ...
    public function decodeJsonCastedAttributes($data)
    {
        ...
        foreach ($fields as $field) {
            // echo $field->field_type->name." - ";
            if(isset($field->field_type->name) && in_array($field->field_type->name, ['Month','Datetime_picker','Date_picker'])) {
                ...
                } else if(in_array($field->field_type->name ,['Datetime_picker']) && isset($data[$field->name]) && $data[$field->name] != "") {
                    $data[$field->name] = new \MongoDB\BSON\UTCDateTime(strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                } else if($field->field_type->name == 'Date_picker' && isset($data[$field->name]) && $data[$field->name] != "") {
                    // echo (strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                    // $utcdatetime = new \MongoDB\BSON\UTCDateTime(strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                    // $datetime = $utcdatetime->toDateTime();
                    // var_dump($datetime);
                    // exit;
                    $data[$field->name] = new \MongoDB\BSON\UTCDateTime(strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                }
            } else if(isset($field->field_type) && in_array($field->field_type, ['Multiselect','Select2_multiple','Select2_from_array_multiple','Select2_from_ajax_multiple','Checkbox'])) {
                if($field->field_type == 'Month' && isset($data[$field->name]) && $data[$field->name]) {
                    $data[$field->name] = \Carbon::parse('01-'.$data[$field->name])->format('Y-m-d');
                // } else if(in_array($field->field_type ,['Datetime']) && isset($data[$field->name])) {
                //     $data[$field->name] = \CustomHelper::date_format($data[$field->name], 'data_save_simpel_with_time');
                } else if(in_array($field->field_type ,['Datetime_picker']) && isset($data[$field->name]) && $data[$field->name] != "") {
                    $data[$field->name] = new \MongoDB\BSON\UTCDateTime(strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                } else if($field->field_type == 'Date_picker' && isset($data[$field->name]) && $data[$field->name] != "") {
                    // echo (strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                    // $utcdatetime = new \MongoDB\BSON\UTCDateTime(strtotime(CustomHelper::date_format($data[$field->name],'data_save')) * 1000);
                    // $datetime = $utcdatetime->toDateTime();
                    // var_dump($datetime);
            ...
            if(isset($field->field_type->name) && in_array($field->field_type->name, ['Multiselect','Select2_multiple','Select2_from_array_multiple','Select2_from_ajax_multiple','Checkbox'])) {
            ...
            if(isset($field->field_type->name) && in_array($field->field_type->name, ['Json'])) {
                $arr = [];
                foreach(json_decode($field->json_values) as $json) {
                    $field_name = $field->name.'_'.$json;
                    
                    if(isset($data) && array_key_exists($field_name, $data)) {
